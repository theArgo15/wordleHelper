...
def weightWords(filename, rankdf):
    pass
...
def bestLocations(filename, position):
    ...
    with open(filename, "r") as allWords:
        with open("mutable.txt", "a") as mutable:
            ...
            sort_alphabet = sorted(
                locationAlphabet.items(), key=lambda x: x[1], reverse=True
            )
...
weightedList = pd.read_csv("words_with_weights.csv")
...
df = pd.read_csv("words_with_weights.csv")
remainingWords = pd.read_csv("less_words.txt", names=["word"])
remainingWords = remainingWords.word.to_list()
valueLst = []
for word in remainingWords:
    if len(df.loc[df["word"] == word, "count"].values) == 1:
        valueLst.append(df.loc[df["word"] == word, "count"].values[0])
    else:
        valueLst.append(0)
potentialAnswers = pd.DataFrame({"word": remainingWords, "popularity count": valueLst})
print(
    potentialAnswers.sort_values("popularity count", ascending=False).reset_index(
        drop=True
    )
)
